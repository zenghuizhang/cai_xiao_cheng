  ...
  <view class="calc">
    <view class="calc-card cx-card">
      ...
      <view class="field">
        <view class="field-head">
          <text class="field-label">每月定投</text>
          <text class="field-val">{{ formatYuan(monthly, 0) }}</text>
        </view>
        <slider
          :value="monthly"
          :min="0"
          ...
          block-color="#ff8c42"
          @change="(e) => (monthly = e.detail.value)"
        />
      </view>
      ...
      <view class="field">
        <view class="field-head">
          <text class="field-label">坚持年数</text>
    ...
    <view class="result-card cx-card">
      ...
      <view class="result-grid">
        <view class="rg-item">
          <text class="rg-num">{{ formatYuan(paidIn, 0) }}</text>
          <text class="rg-label">累计投入</text>
        </view>
        <view class="rg-item">
          <text class="rg-num pos">{{ formatSigned(gain) }}</text>
          <text class="rg-label">累计收益（元）</text>
        </view>
      </view>
    </view>
    ...
    <view class="series cx-card">
      ...
      <view v-for="row in series" :key="row.year" class="series-row">
        ...
        <text class="c-gain pos">{{ formatSigned(row.gain) }}</text>
      </view>
    </view>

    <Disclaimer variant="warn" title="教学说明" icon="⚠️" text="定投「微笑曲线」靠的是长期纪律，并非稳赚。以上为固定假设收益率下的数学推演，真实市场会波动甚至阶段性亏损。不构成任何投资建议。" />
  </view>
</template>
...
import { totalFutureValue, dcaYearlySeries } from '@/utils/finance'
...
const fv = computed(() => totalFutureValue({ principal, monthly: monthly.value, annualRate: annualRate.value, years: years.value }))
const paidIn = computed(() => monthly.value * 12 * years.value)
const gain = computed(() => fv.value - paidIn.value)
const series = computed(() =>
  dcaYearlySeries({ principal, monthly: monthly.value, annualRate: annualRate.value, years: years.value })
)
</script>
